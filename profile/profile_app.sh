...
function get_profile_name() {
  ...
  profile_name=$(docker run --rm --entrypoint sh -v "${PWD}":/workdir mikefarah/yq:3.3.4 -c "cd /workdir && yq r profile.yaml name" | xargs | tr -d '\r' | sed -e 's/ /_/g')
  exit_code=$?
...
function yaml_to_json() {
  ...
  docker run --rm --entrypoint sh -v "${PWD}":/workdir mikefarah/yq:3.3.4 -c "cd /workdir && yq r -j --prettyPrint profile.yaml" > profile.json
  check_status $?
}
